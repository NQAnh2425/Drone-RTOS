...
{
	if (  HAL_UARTEx_ReceiveToIdle_DMA(huart, SBUS_data_raw, sizeof(SBUS_data_raw)) != HAL_OK)
	{
...
int8_t SBUS_find_frame(uint8_t *buf)
{
...
bool SBUS_is_frame_lost( uint8_t *buf)
{
    return (buf[(header_pos + 23)%SBUS_FRAME_LEN] & (1 << 2)) != 0;
}

bool SBUS_is_failsafe( uint8_t *buf)
{
...
{
	if(SBUS_find_frame(buf) == -1)
		return SBUS_FRAME_NOTFOND;
	if (SBUS_is_frame_lost(buf))
		return SBUS_ERR_FRAME_LOST;
	if (SBUS_is_failsafe(buf))
